...
/**
 * EA JavaAcademy Monopoly Server Class Stable server for the Monopoly game
 * application running using Java Sockets each client runs on a separate thread
 * all client sockets listens in the same time for requests messages can be sent
 * to all clients, publically or private
 * 
 * @author achesnoiu
...
public final class Server implements Runnable {
	...
	/**
	 * @author achesnoiu private constructor for respecting the singleton
	 *         pattern
	 */
	private Server() {
	...
	/**
	 * @author achesnoiu sets the requires clients number
	 * @param requiredClients
	 *            must be an integer in [2,8]
	 * @throws Exception
	 *             if requiredClient is not valid
	 */
	...
	/**
	 * @author achesnoiu start the server
	...
	/**
	 * @author achesnoiu sets the listening port
	 * @param listeningPort
	 *            must be an integer between 8000 and 9999
	 */
	...
	/**
	 * @author achesnoiu stops the server, and exista the application
	 */
	...
	/**
	 * @author achesnoiu generates serializable vector with all the current
	 *         connected players
	 */
	private static void generateSerializablePlayerVector() {
		serializablePlayers
				.addAll(clients.stream().map(player -> new SerializablePlayer(player.getUsername(), player.getToken()))
						.collect(Collectors.toList()));
		System.out.println("SerializablePlayer vector generated!");
	}

	/**
	 * @author achesnoiu gives permission to turn to next player
	 * @see Player
	 */
	...
	/**
	 * @author achesnoiu sends MessageCodes.GAME_READY_TO_START to all the
	 *         connected players sends NUMBER_OF_PLAYERS to all the connected
	 *         players sends CONNECTED_USERS_VECTOR to all the connected players
	...
	/**
	 * @author achesnoiu informs all players about username's player location's
	 *         update
	 * @param username
	...
	/**
	 * @author achesnoiu server thread run method
	 */
	@Override
	public void run() {
		try {
			...
			while (isRunning) {
				...
				if (currentConnectedClients < requiredClients)
					...
					objectOutputStream
							.writeObject(new Message(MessageCodes.CONNECTION_REFUSED, "Maximum connexions reached."));
					objectInputStream.close();
					objectOutputStream.close();
					socket.close();
				}
...
>>>>>>> branch 'master' of https://github.com/stingster/MonopolyServer.git
